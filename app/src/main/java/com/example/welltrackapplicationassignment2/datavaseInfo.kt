        ...
        cursor.use { // Ensures the cursor is closed automatically
            while (it.moveToNext()) {
                val program = StretchProgram(
                    ...
                    averageWorkoutsPerDay = it.getDouble(it.getColumnIndexOrThrow(COLUMN_AVERAGE_WORKOUTS_PER_DAY)),
                    totalCaloriesBurned = it.getInt(it.getColumnIndexOrThrow(COLUMN_TOTAL_CALORIES_BURNED)),
    ...
    fun getEnrolledPrograms(): List<StretchProgram> {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val timeInSeconds = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_TIME_IN_SECONDS))
                val caloriesBurned = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_CALORIES_BURNED))
                val workoutsCompleted = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_WORKOUTS_COMPLETED))
                val secondsSpent = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_SECONDS_SPENT))
                val averageWorkoutsPerDay = cursor.getDouble(cursor.getColumnIndexOrThrow(COLUMN_AVERAGE_WORKOUTS_PER_DAY))
                val totalCaloriesBurned = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_TOTAL_CALORIES_BURNED))
                val enrolledStatus = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_ENROLLED_STATUS))
                val isPremium = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_PROGRAM_PREMIUM_STATUS)) == 1
    ...
    fun getWorkoutBundles(): List<Pair<WorkoutBundle, List<StretchProgram>>> {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val description = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_BUNDLE_DESCRIPTION))
                val image = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_BUNDLE_IMAGE))
                val isPremium = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_BUNDLE_PREMIUM_STATUS)) == 1
            ...
            """.trimIndent()
                ...
                if (workoutsCursor.moveToFirst()) {
                    do {
                        val program = StretchProgram(
                            id = workoutsCursor.getInt(workoutsCursor.getColumnIndexOrThrow(COLUMN_ID)),
                            title = workoutsCursor.getString(workoutsCursor.getColumnIndexOrThrow(COLUMN_TITLE)),
                            description1 = workoutsCursor.getString(workoutsCursor.getColumnIndexOrThrow(COLUMN_DESCRIPTION1)),
                            description2 = workoutsCursor.getString(workoutsCursor.getColumnIndexOrThrow(COLUMN_DESCRIPTION2)),
                            image = workoutsCursor.getString(workoutsCursor.getColumnIndexOrThrow(COLUMN_IMAGE)),
                            timeInSeconds = workoutsCursor.getInt(workoutsCursor.getColumnIndexOrThrow(COLUMN_TIME_IN_SECONDS)),
                            caloriesBurned = workoutsCursor.getInt(workoutsCursor.getColumnIndexOrThrow(COLUMN_CALORIES_BURNED)),
                            isPremium = workoutsCursor.getInt(workoutsCursor.getColumnIndexOrThrow(COLUMN_PROGRAM_PREMIUM_STATUS)) == 1
                        )
    ...
    fun getWorkoutById(workoutId: Int): StretchProgram? {
        ...
        if (cursor.moveToFirst()) {
            workout = StretchProgram(
                ...
                workoutsCompleted = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_WORKOUTS_COMPLETED)),
                secondsSpent = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_SECONDS_SPENT)),
                averageWorkoutsPerDay = cursor.getDouble(cursor.getColumnIndexOrThrow(COLUMN_AVERAGE_WORKOUTS_PER_DAY)),
                totalCaloriesBurned = cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_TOTAL_CALORIES_BURNED)),
